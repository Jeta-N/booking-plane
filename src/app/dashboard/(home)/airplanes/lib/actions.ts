...
export async function updateAirplane(prevState: unknown, id: string, formData: FormData): Promise<ActionResult> {
    ...
    if (image.size > 0) {
        ...
        const airplane = await prisma.airplane.findFirst({ where: { id: id }, select: { image: true } })
